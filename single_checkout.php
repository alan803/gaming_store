...
// Fetch product
$stmt = $conn->prepare("SELECT id, name, price, image_name, stock FROM products WHERE id = ? LIMIT 1");
if (!$stmt) {
    die('DB error');
}
...
// Clamp quantity to available stock (at least 1)
...
$order_payload = [
    'amount' => $amount_paise,
    'currency' => 'INR',
    'receipt' => 'single_rcpt_' . time() . '_' . $user_id . '_' . $product_id,
    'payment_capture' => 1,
];
...
<body class="min-h-screen">
        ...
        <div class="container mx-auto px-4 relative z-10">
            <div class="text-center mb-10">
                <h1 class="text-4xl md:text-5xl font-bold mb-4">
                    ...
                    <span class="bg-gradient-to-r from-indigo-600 to-purple-600 bg-clip-text text-transparent">Now</span>
                </h1>
                <p class="text-gray-600">Complete payment to place your order</p>
            </div>
    ...
    (function(){
        ...
        payBtn.addEventListener('click', function (e) {
            ...
            const options = {
                key: <?php echo json_encode($key_id); ?>,
                amount: <?php echo json_encode($amount_paise); ?>,
                currency: 'INR',
                name: 'NexusGear',
                description: 'Buy Now Payment',
                order_id: <?php echo json_encode($razorpay_order_id); ?>,
                handler: function (response){
                    const body = `razorpay_payment_id=${encodeURIComponent(response.razorpay_payment_id)}&razorpay_order_id=${encodeURIComponent(response.razorpay_order_id)}&razorpay_signature=${encodeURIComponent(response.razorpay_signature)}&product_id=<?php echo (int)$product_id; ?>&quantity=<?php echo (int)$quantity; ?>`;
                    fetch('verify_payment_single.php', {
                        method: 'POST',
                    ...
                    .then(async r => {
                        ...
                        let data;
                        try { data = JSON.parse(text); } catch (e) { data = { success: false, message: text || 'Invalid JSON response from server' }; }
                    ...
                    .catch(err => {
                        alert('Server error while verifying payment: ' + (err && err.message ? err.message : err));
                    });
                },
                prefill: {
                    name: <?php echo json_encode(isset($_SESSION['auth_username']) ? $_SESSION['auth_username'] : ''); ?>,
                    email: <?php echo json_encode(isset($_SESSION['auth_user_email']) ? $_SESSION['auth_user_email'] : ''); ?>
                },
                theme: { color: '#4f46e5' }
            };
